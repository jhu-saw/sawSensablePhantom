...
void mtsSensableHD::Run(void)
{
    ...
    if (HD_DEVICE_ERROR(error = hdGetError())) {
        ...
        if (hduIsSchedulerError(&error)) {
            CMN_LOG_RUN_ERROR << "mtsSensableHDCallback: Scheduler error detected\n";
...
mtsSensableHD::mtsSensableHD(const std::string & taskName):
...
mtsSensableHD::mtsSensableHD(const char * taskName,
...
void mtsSensableHD::SetInterfaces(const std::string & firstDeviceName,
                                  const std::string & secondDeviceName,
                                  bool firstDeviceForcesEnabled,
                                  bool secondDeviceForcesEnabled)
{
...
void mtsSensableHD::Create(void * data)
{ 
    unsigned int index = 0;
    const unsigned int end = this->DevicesVector.size();
